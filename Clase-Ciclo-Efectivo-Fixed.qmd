...
author: Docente Carlos Correa Iniguez
format:
  revealjs:
    theme: solarized
    css: duoc_uc.css
...
Los costos de mantenimiento o costos de transporte de existencias son **costos fijos** incurridos al almacenar las existencias:

- **Alquiler de almacén**: Costos de espacio físico para almacenamiento
<br><br>
- **Impuestos y seguros**: Protección legal y fiscal del inventario
<br><br>
- **Depreciación o merma**: Pérdida de valor por obsolescencia o deterioro
<br><br>
- **Gestión de inventario**: Costos operativos de administrar las existencias
<br><br>
- **Notación en EOQ**: Se denota como **H** (costo de mantenimiento por unidad por período)
...
- **Demanda anual (D)**: 12,000 unidades/año
- **Costo por orden (K)**: \$60 por orden
- **Costo de mantención por unidad-año (h)**: \$3

**Objetivo:** Mostrar cómo varían los costos según el tamaño del pedido (Q).

**Donde:**

- **D**: Demanda anual (unidades por año)
- **K**: Costo por orden (\$ por orden)
- **h**: Costo de mantención por unidad por año (\$ por unidad por año)
- **Q**: Tamaño del pedido (unidades por pedido)

---
### Fórmulas Clave

**Costo de ordenar por año:**
$$
C_{\text{orden}}(Q) = K \cdot \frac{D}{Q}
$$

**Costo de mantención por año:**
$$
C_{\text{mant}}(Q) = h \cdot \frac{Q}{2}
$$

**Costo total (sin costo de compra):**
$$
...
### Comportamiento de los Costos en el Modelo EOQ

### Observaciones del Ejemplo

**Trade-off entre costos:**

- Al **duplicar Q** (de 600 a 1,200 unidades):
  - El inventario promedio aumenta de 300 a 600 unidades
  - El costo de mantención se **duplica**: \$900 → \$1,800
  - Las órdenes anuales disminuyen de 20 a 10
  - El costo de ordenar se **reduce a la mitad**: \$1,200 → \$600
...
**EOQ teórico:**
$$
Q^* = \sqrt{\frac{2 \cdot K \cdot D}{h}} = \sqrt{\frac{2 \cdot 60 \cdot 12,000}{3}} = \sqrt{480,000} \approx 693 \text{ unidades}
$$

**Observación:** Q = 700 unidades está muy cerca del óptimo teórico y presenta el **costo total mínimo** en la tabla (\$2,079).

---

### Idea Clave del Trade-off

- **Pedidos pequeños (Q bajo)**:
  - ✓ Menor costo de mantención (menos inventario)
  - ✗ Mayor costo de ordenar (más órdenes frecuentes)

- **Pedidos grandes (Q alto)**:
  - ✗ Mayor costo de mantención (más inventario promedio = Q/2)
  - ✓ Menor costo de ordenar (menos órdenes anuales)

**El EOQ encuentra el equilibrio perfecto** donde ambos costos se minimizan.
...
**Lectura del gráfico:** 
- Se **reordena** cuando el inventario cae a **r = d·L** (línea punteada horizontal)
- Tras **L** días, llega el pedido: el nivel vuelve instantáneamente a $Q^*$
- El patrón se repite formando un comportamiento "tipo sierra"
...
- EOQ equilibra $\frac{K \cdot D}{Q}$ (ordenar) con $\frac{h \cdot Q}{2}$ (mantener).

- Óptimo: 
  $$Q^* = \sqrt{\frac{2 \cdot K \cdot D}{h}}$$
  
- En $Q^*$, costos de ordenar y mantener **se igualan**.
...
- Hillier & Lieberman, *Introducción a la Investigación de Operaciones* (cap. de inventarios).
- Hadley & Whitin, *Analysis of Inventory Systems*.
- Anderson et al., *Métodos Cuantitativos para los Negocios*.

---

### Ejemplo Práctico: Viña Santa Rita
